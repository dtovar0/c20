/* Teams DETAIL MODULE - LOGIC DECOUPLING */
...
function reprocessDuplicates(taskId, alreadyHasRetry = false, retryId = '') {
    if (alreadyHasRetry) {
        ...
        if (modal && label && btn) {
            ...
            btn.href = `/api/teams/detail/${retryId}`;
...
async function executeReprocess(taskId) {
    ...
    try {
        const res = await fetch(`/api/teams/reprocess_duplicates/${taskId}/`, { method: 'POST' });
        const data = await res.json();
        
        if (data.status === 'success') {
            closeReprocessModal();
            // Notificamos éxito con el estilo de la plataforma (puedes usar un toast si tienes, si no alert por ahora)
            location.href = `/api/teams/detail/${data.task_id}?tour=true`;
        } else {
