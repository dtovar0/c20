...
/**
 * MODULE: Teams Operational Terminal Controller (DataTables Powered)
 * Dedicated logic for rendering Teams tasks with DataTables integration.
 */
 ...
 *
 * En Teams cada número tiene solo dos desenlaces por tabla:
 ...
 */$(document).ready(function() {
    ...
    $('#auditSearch').on('input', function() {
        if (teamsDataTable) {
            teamsDataTable.search(this.value).draw();
        }
    });
    ...
    $('.filter-opt').on('click', function() {
        ...
        if (teamsDataTable) {
            ...
            teamsDataTable.column(6).search(status).draw();
        }
    ...
    // Select All logic for Teams
    $(document).on('change', '#selectAllTeams', function() {
        const isChecked = this.checked;
        if (!teamsDataTable) return;
        const nodes = teamsDataTable.rows({ page: 'current' }).nodes().to$();
...
function initTeamsDataTable() {
        ...
        columns: [
            ...
            { 
                ...
                render: (data, type, row) => {
                    ...
                    // Teams no tiene modo Force ni modos de llamada: la acción es alta o baja
            ...
            { 
                ...
                render: (data, type, row) => {
                    ...
                    return `
                        <div class="flex items-center justify-center h-full">
                            <a href="/api/teams/detail/${data}" target="_blank" class="p-2 hover:bg-primary/10 text-primary/40 hover:text-primary rounded-xl transition-all" data-nx-tooltip="Ver Detalles">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                            </a>
                        </div>`;
                }
    ...
    tableEl.on('click', '.nexus-check-trigger', function(e) {
        ...
        const totalOnPage = teamsDataTable.rows({ page: 'current' }).data().length;
        const selectedOnPage = teamsDataTable.rows({ page: 'current' }).nodes().to$().filter('.nexus-row-selected').length;
        $('#selectAllTeams').prop('checked', selectedOnPage === totalOnPage && totalOnPage > 0);

        updateTeamsActions();
    });

    window.activeNexusTable = teamsDataTable;
}
...
function updateTeamsActions() {
    ...
    if (tr.length === 1) {
        const data = teamsDataTable.row(tr).data();
...
$(window).on('resize', () => {
    if (teamsDataTable) teamsDataTable.draw(false);
});
