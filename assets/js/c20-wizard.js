...
function openWizard() {
    const modal = document.getElementById('c20WizardModal');
    if (modal) {
        resetWizardForm();
...
function resetWizardForm() {
    document.querySelectorAll('#c20WizardModal input, #c20WizardModal textarea').forEach(el => {
        el.value = '';
    ...
    const forceToggle = document.getElementById('c20ForceToggle');
    ...
    const border = document.getElementById('c20FileInput') ? document.getElementById('c20FileInput').parentElement : null;
...
function validateCurrentStep() {
    ...
    if (currentWizardStep === 1) {
        ...
        if (isManual) {
            ...
            const fileInput = document.getElementById('c20FileInput');
...
function triggerShakeEffect() {
    const wizardBox = document.querySelector('#c20WizardModal > div:last-child');
...
function updateWizardUI() {
    ...
    const forceContainer = document.getElementById('c20ForceContainer');
    if (forceContainer) {
...
async function finalizeWizard() {
    ...
    const forceTask = document.getElementById('c20ForceToggle').checked;
    ...
    const progressModal = document.getElementById('c20ProgressModal');
    ...
    if (progressModal) {
        ...
        try {
            ...
            if (!isManual && fileInput && fileInput.files.length > 0) {
                ...
                const uploadRes = await fetch('/api/c20/upload', { method: 'POST', body: fileFormData });
            ...
            const createRes = await fetch('/api/c20/create', {
                method: 'POST',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (mainBtn && document.getElementById('c20-view-marker')) {
...
function resetScheduleWizardForm() {
    ...
    const scheduleForceToggle = document.getElementById('scheduleC20ForceToggle');
...
async function finalizeScheduleWizard() {
    ...
    const forceTask = document.getElementById('scheduleC20ForceToggle').checked;
    ...
    if (progressModal) {
        ...
        try {
            if (!isManual && fileInput && fileInput.files.length > 0) {
                ...
                const up = await fetch('/api/c20/upload', { method: 'POST', body: fd });
                const ur = await up.json();
                if (ur.status === 'success') taskPayload.datos = ur.filename;
            }

            const res = await fetch('/api/c20/create', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(taskPayload)
            });
...
async function openModifyModal(taskId) {
    ...
    try {
        ...
        const res = await fetch(`/api/c20/list`);
...
async function confirmModifyAction() {
    ...
    try {
        const res = await fetch(`/api/c20/job/update/${currentModifyJobId}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (result.status === 'success') {
            ...
            closeModifyModal();
            if (typeof fetchC20Data === 'function') fetchC20Data();
            if (c20DataTable) c20DataTable.ajax.reload();
        } else {
