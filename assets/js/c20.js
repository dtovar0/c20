...
/**
 * MODULE: C20 Operational Terminal Controller (DataTables Powered)
 * Dedicated logic for rendering C20 tasks with DataTables integration.
 */
 ...
 *
 * En C20 cada número tiene solo dos desenlaces por tabla:
 *   OK   -> se aplicó la operación (alta en 'add', baja en 'del')
 ...
 *
 * La barra se calcula sobre OFC2CODE, que es la tabla por la que pasa cada
 * número exactamente una vez; SNPANAME y TOFCNAME trabajan sobre ladas y series
 * deduplicadas, así que sus totales no son comparables con el tamaño del lote.
 */
function generateTaskGraphic(resumen, taskType) {
    ...
    const colorOk = taskType === 'add' ? c.ok : (c.del || '#6366f1');
    const colorFail = c.dup || '#f59e0b';

    const ok = resumen.ok || 0;
    const fail = resumen.fail || 0;
    const processed = ok + fail;

    const okPct = (ok / resumen.total) * 100;
    const failPct = (fail / resumen.total) * 100;
    const currentPct = Math.min((processed / resumen.total) * 100, 100);
    const pendingPct = Math.max(0, 100 - currentPct);

    const verbo = taskType === 'add' ? 'Alta' : 'Baja';
    const motivo = taskType === 'add' ? 'ya existía' : 'no existía';

    // Desglose por tabla en el tooltip: revela en qué paso se quedó una carga
    const detalle = ['snpaname', 'tofcname', 'ofc2code', 'dnscrn']
        .filter(t => resumen[t] && resumen[t].total)
        .map(t => `${t.toUpperCase()}: ${resumen[t].ok}/${resumen[t].total}`)
        .join(' | ');

    return `
        <div class="flex flex-col gap-1.5 w-full max-w-[130px] mx-auto" data-nx-tooltip="${detalle || 'Sin datos'}">
            <div class="h-4 w-full rounded-lg overflow-hidden flex shadow-inner border border-white/5" style="background:rgba(148,163,184,0.15)">
                <div class="h-full transition-all duration-700 ease-out" style="width:${okPct}%;background:${colorOk}" title="${verbo} aplicada: ${ok}"></div>
                <div class="h-full transition-all duration-700 ease-out" style="width:${failPct}%;background:${colorFail}" title="Sin aplicar (${motivo}): ${fail}"></div>
 ...
 */$(document).ready(function() {
    ...
    $('#auditSearch').on('input', function() {
        if (c20DataTable) {
            c20DataTable.search(this.value).draw();
        }
    });
    ...
    $('.filter-opt').on('click', function() {
        ...
        if (c20DataTable) {
            ...
            c20DataTable.column(6).search(status).draw();
        }
    ...
    $('#refreshAudit').on('click', function() {
        if (c20DataTable) {
            c20DataTable.ajax.reload();
        }
    });

    // Select All logic for C20
    $(document).on('change', '#selectAllC20', function() {
        const isChecked = this.checked;
        if (!c20DataTable) return;
        const nodes = c20DataTable.rows({ page: 'current' }).nodes().to$();
...
/**
 * Initializes DataTables for C20
 */
function initC20DataTable() {
    const tableEl = $('#auditTableBody').closest('table');
    if (!tableEl.length) return;

    c20DataTable = tableEl.DataTable({
        retrieve: true,
        ajax: {
            url: '/api/c20/list',
            dataSrc: (json) => {
        ...
        columns: [
            ...
            { 
                ...
                render: (data, type, row) => {
                    ...
                    return `<div class="flex items-center justify-center h-full min-w-0 overflow-hidden">${generateTaskGraphic(data, row.tarea)}</div>`;
                }
            },
            { 
                data: 'estado', 
                ...
                render: (data, type, row) => {
                    ...
                    // C20 no tiene modo Force ni modos de llamada: la acción es alta o baja
                    const actionStyle = getNexusBadgeStyle(isAdd ? c.ok : c.del);
            ...
            { 
                ...
                render: (data, type, row) => {
                    const state = (row.estado || '').toLowerCase();
                    const isFinished = (state.startsWith('completad') || state.includes('error') || state.includes('terminad'));
                    ...
                    return `
                        <div class="flex items-center justify-center h-full">
                            <a href="/api/c20/detail/${data}" target="_blank" class="p-2 hover:bg-primary/10 text-primary/40 hover:text-primary rounded-xl transition-all" data-nx-tooltip="Ver Detalles">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                            </a>
                        </div>`;
                }
    ...
    tableEl.on('click', '.nexus-check-trigger', function(e) {
        ...
        const totalOnPage = c20DataTable.rows({ page: 'current' }).data().length;
        const selectedOnPage = c20DataTable.rows({ page: 'current' }).nodes().to$().filter('.nexus-row-selected').length;
        $('#selectAllC20').prop('checked', selectedOnPage === totalOnPage && totalOnPage > 0);

        updateC20Actions();
    });

    window.activeNexusTable = c20DataTable;
}

function updateC20Actions() {
    const tr = $('#auditTableBody tr.nexus-row-selected');
    const modifyBtn = $('#modifyTaskBtn');
    
    if (tr.length === 1) {
        const data = c20DataTable.row(tr).data();
...
$(window).on('resize', () => {
    if (c20DataTable) c20DataTable.draw(false);
});
