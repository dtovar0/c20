...
function openWizard() {
    const modal = document.getElementById('teamsWizardModal');
    if (modal) {
        resetWizardForm();
...
function resetWizardForm() {
    document.querySelectorAll('#teamsWizardModal input, #teamsWizardModal textarea').forEach(el => {
        el.value = '';
    ...
    const forceToggle = document.getElementById('teamsForceToggle');
    ...
    const border = document.getElementById('teamsFileInput') ? document.getElementById('teamsFileInput').parentElement : null;
...
function triggerShakeEffect() {
    const wizardBox = document.querySelector('#teamsWizardModal > div:last-child');
...
function updateWizardUI() {
    ...
    const forceContainer = document.getElementById('teamsForceContainer');
    if (forceContainer) {
...
async function finalizeWizard() {
    ...
    const forceTask = document.getElementById('teamsForceToggle').checked;
    ...
    const progressModal = document.getElementById('teamsProgressModal');
    ...
    if (progressModal) {
        ...
        try {
            ...
            if (!isManual && fileInput && fileInput.files.length > 0) {
                ...
                const uploadRes = await fetch('/api/teams/upload', { method: 'POST', body: fileFormData });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (mainBtn && document.getElementById('teams-view-marker')) {
...
function resetScheduleWizardForm() {
    ...
    const scheduleForceToggle = document.getElementById('scheduleTeamsForceToggle');
...
async function finalizeScheduleWizard() {
    ...
    const forceTask = document.getElementById('scheduleTeamsForceToggle').checked;
    ...
    if (progressModal) {
        ...
        try {
            if (!isManual && fileInput && fileInput.files.length > 0) {
                ...
                const up = await fetch('/api/teams/upload', { method: 'POST', body: fd });
...
async function confirmModifyAction() {
    ...
    try {
        const res = await fetch(`/api/teams/job/update/${currentModifyJobId}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (result.status === 'success') {
            ...
            closeModifyModal();
            if (typeof fetchTeamsData === 'function') fetchTeamsData();
            if (teamsDataTable) teamsDataTable.ajax.reload();
        } else {
