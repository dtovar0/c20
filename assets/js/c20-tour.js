/**
 * NEXUS PREMIUM GUIDED TOUR - C20 TERMINAL (V8)
 */
...
class C20NexusTour {
    ...
    init() {
        console.log('🏁 Initializing C20 Tour Elements...');
    ...
    start() {
        console.log('🚀 Starting C20 Guided Tour');
    ...
    showStep() {
        ...
        console.log(`[C20-TOUR] Showing step ${this.currentStep + 1}/${this.steps.length}: ${step.title}`);
        const target = document.querySelector(step.target);
        console.log(`[C20-TOUR] Target element for '${step.target}':`, target);
        
        if (target) {
            const rect = target.getBoundingClientRect();
            console.log(`[C20-TOUR] Target rect:`, rect);
            const padding = 10;
            ...
            const arrowLeft = `<svg class="nx-tour-arrow ${this.currentStep === 0 ? 'disabled' : ''}" onclick="window.c20Tour.prev()" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M15 19l-7-7 7-7"></path></svg>`;
            const arrowRight = `<svg class="nx-tour-arrow" onclick="window.c20Tour.next()" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M9 5l7 7-7 7"></path></svg>`;
            ...
            console.warn(`[C20-TOUR] Target not found: ${step.target}. Moving to next step.`);
            this.next();
        }
    }
}
...
window.logAuditTour = new C20NexusTour([
    {
        type: 'Buscador',
        target: '#logSearch',
...
(function() {
    window.c20Tour = new C20NexusTour([
        {
            type: 'Buscador',
            target: '#auditSearch',
        ...
        },
        {
            ...
            content: 'Etiqueta lógica de destino enviada al C20. Define el canal de salida de los datos procesados.'
        },
        {
            type: 'Columna + Iconos',
    ...
    if (helpBtn) {
        console.log('✅ C20 Help Button Found. Attaching Listener.');
        helpBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            window.c20Tour.start();
        });
    }
})();
