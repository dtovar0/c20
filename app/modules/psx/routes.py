...
def update_or_reprocess_job(job_id):
    ...
    # Estado global del job (basado en sus tareas): determina si editamos o clonamos
    ...
    try:
        ...
        if action != 'cancel':
            active_tasks = PSX5KTask.query.filter(
                PSX5KTask.job_id == job_id, PSX5KTask.estado == 'Ejecutando'
            ).count()
            if active_tasks > 0:
