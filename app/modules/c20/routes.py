...
def task_detail(task_id):
    ...
    try:
        ...
        history = C20History.query.filter_by(task_id=task_id).order_by(C20History.fecha.desc()).all()
...
def download_duplicates(task_id):
    ...
    try:
        ...
        history = C20History.query.filter_by(task_id=task_id, estado='FAIL').all()
        if not history:
            return jsonify({"status": "error", "message": "No hay registros sin aplicar en esta tarea"}), 404

        csv_content = "numero,zona,fecha\n"
        for item in history:
            csv_content += f"{item.numero},{item.zona or ''},{item.fecha}\n"
...
def reprocess_duplicates(task_id):
    ...
    try:
        ...
        pendientes = C20History.query.filter_by(task_id=task_id, estado='FAIL').all()
        if not pendientes:
...
def search_history():
    """
    Búsqueda profunda en la tabla de historial (c20_history)
    """
    try:
        ...
        results = C20History.query.filter(
            (C20History.numero.like(f'%{query_str}%')) |
            (C20History.zona.like(f'%{query_str}%')) |
            (C20History.estado.like(f'%{query_str}%'))
        ).order_by(C20History.fecha.desc()).limit(100).all()
    ...
    try:
        ...
        if action == 'activate':
            ...
            for t in job.tasks:
                ...
                if not t.datos:
                    recovery = C20History.query.filter_by(task_id=t.id).all()
                    if recovery:
        ...
        if finished and action == 'modify':
            ...
            for old_task in job.tasks:
                ...
                if not task_data:
                    recovery = C20History.query.filter_by(task_id=old_task.id).all()
                    if recovery:
