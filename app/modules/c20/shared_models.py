"""
Modelos compartidos entre las secciones de la plataforma C20 (C20 y Teams).

Ambas secciones atacan el mismo nodo y las mismas tablas del conmutador, así que
el estado del equipo y el rastro de cada número son únicos: no tiene sentido que
...
# Tablas del equipo C20 que produce cada ejecución, en orden jerárquico
...
class C20History(db.Model):
    ...
    __table_args__ = (
        ...
        db.Index('ix_c20_history_seccion_task', 'seccion', 'task_id'),
    )
...
# Estas cuatro tablas reflejan QUÉ ESTÁ DADO DE ALTA ahora mismo en el C20:
...
# representa eliminando la fila. El rastro temporal vive en c20_history.
#
# Son compartidas porque el estado del C20 es uno solo: un número dado de
# alta desde C20 ya existe para Teams, y viceversa.

class C20Snpaname(db.Model):
    """
    ESPEJO: ladas dadas de alta en el C20 (tabla SNPANAME del nodo).
...
class C20Tofcname(db.Model):
    """ESPEJO: series dadas de alta en el C20 (tabla TOFCNAME del nodo)."""
...
def make_detail_columns(prefix_table):
    """
    Genera las columnas de contadores de una tabla del C20.
