...
with app.app_context():
    ...
    print("🔄 Reiniciando tabla PSX5K para incluir columna 'tipo'...")
    ...
    example_tasks = [
        PSX5K(id=1, usuario="OPERADOR_ALPHA", accion="add", estado="Terminada", tipo="Archivo",
              fecha_inicio=now - timedelta(days=1), fecha_fin=now - timedelta(hours=23),
              datos='{"job": "LOTE_77", "node": "NEXUS_MAIN"}'),
              
        PSX5K(id=2, usuario="SISTEMA_AUTO", accion="delete", estado="Terminada", tipo="Manual",
              fecha_inicio=now - timedelta(hours=5), fecha_fin=now - timedelta(hours=4),
              datos='{"cleanup": "OLD_TEMP_LOGS"}'),
              
        PSX5K(id=3, usuario="ADMIN_NEXUS", accion="add", estado="Ejecutando", tipo="Archivo",
              fecha_inicio=now - timedelta(minutes=20), fecha_fin=None,
              datos='{"process": "NETWORK_SYNC", "progress": "45%"}'),
              
        PSX5K(id=4, usuario="TECNICO_09", accion="add", estado="Programada", tipo="Manual",
              fecha_inicio=now + timedelta(hours=2), fecha_fin=None,
              datos='{"schedule": "MIDNIGHT_RUN"}'),
              
        PSX5K(id=5, usuario="ADMIN_NEXUS", accion="delete", estado="Programada", tipo="Archivo",
              fecha_inicio=now + timedelta(days=1), fecha_fin=None,
              datos='{"target": "LEGACY_DATA"}'),
              
        PSX5K(id=6, usuario="SISTEMA_AUTO", accion="add", estado="Pendiente", tipo="Manual",
    ...
    print(f"✅ Se han generado {len(example_tasks)} registros con columna 'tipo'.")
